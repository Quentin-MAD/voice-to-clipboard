  ...
  const stopReadRecording = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || !json.translation || !json.audio) {
        statusQuery.refetch();
        const blockedRead = limitBlockFromCode(
          json.code === "no_credits" ? "no_voice_credits" : json.code,
        );
        if (blockedRead) {
          setLimitBlock(blockedRead);
            ...
            <div className="mb-6 rounded-xl border border-primary/40 bg-primary/5 p-4">
              ...
              <p className="mb-3 text-xs text-muted-foreground">
                ...
                globalement pour que l'enregistrement marche en jeu plein écran, et copie la traduction
                 automatiquement dans le presse-papiers. L'installation et les prochaines mises à jour sont automatiques.
              ...
              <p className="mt-2 text-[11px] text-muted-foreground">
                 Les mises à jour suivantes se feront directement dans l'application.
              </p>
            </div>
          )}
